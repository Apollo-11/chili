...
export const MultiSelect = React.forwardRef((props: MultiSelectProps, ref: React.Ref<HTMLElement>): React.ReactElement => {
  ...
  React.useEffect(() => {
    ...
    if (hasCheckBoxes && !shouldKeepSuggestions) console.warn('Chili MultiSelect: you probably forgot using shouldKeepSuggestions with hasCheckBoxes prop.');
    if (canSelectAll && !shouldKeepSuggestions) console.warn('Chili MultiSelect: you probably forgot using shouldKeepSuggestions with canSelectAll prop.');
  ...
  const { renders: { [COMPONENTS_NAMESPACES.multiSelect]: multiSelectRenders } } = React.useContext(ChiliContext);
